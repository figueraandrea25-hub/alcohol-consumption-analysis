...
tabla_comparativa <- data.frame(
  Indicador = c("Nro. de Países", "Media (Litros)", "Desv. Estándar", "Valor Máximo"),
  Muestra_Completa = c(
...
aplicar_estilo_tabla(tabla_comparativa, "Comparación Estadística: Muestra Completa vs. Muestra Truncada (P90)")
```
...
colnames(tabla_de_intervalos) <- c("Región", "N° Países", "Consumo Medio", "Lím. Inferior", "Lím. Superior", "Margen Error")

aplicar_estilo_tabla(tabla_de_intervalos, "Inferencia Estadística: Estimación del Consumo por Región (IC 95%)")
``` 
\chapter{Análisis de Resultados}

{\small En este capítulo se presentan los hallazgos derivados de la aplicación de los métodos estadísticos descritos previamente sobre el conjunto de datos `alcohol_data.csv`. El análisis se estructura en tres fases fundamentales: la evaluación de la calidad y robustez de los datos post-ETL, la caracterización descriptiva del consumo y la incertidumbre por macro-regiones, y finalmente, la contrastación de significancia mediante la superposición de intervalos de confianza.}
...
\ Para profundizar en el análisis de consistencia, se presenta a continuación un gráfico de dispersión que evalúa la relación bivariada entre el consumo per cápita y la incertidumbre de la medición (ci_width), estratificado por macro-regiones. Esta visualización permite identificar no solo la tendencia global, sino también los comportamientos diferenciales y la concentración de datos en grupos específicos de países.\

```{r grafico-correlacion-por-region}
#calculo del coeficiente pearson
cor_val <- cor(clean_data_alcohol$alcohol_liters_per_capita, 
               clean_data_alcohol$ci_width, use = "complete.obs")
label_cor <- paste("Pearson R = ", round(cor_val, 3))
...
#Etiquetas 
  labs(
    ...
    color = "Macro-Región",
    caption = "Fuente: Elaboración propia basada en microdatos OMS (2015-2019)"
  ) +
...
```
\ El gráfico es un diagrama de dispersión (scatter plot) con una línea de regresión lineal que evalúa la consistencia técnica de los datos reportados para objeto del estudio.

    Coeficiente de Correlación (Pearson R = 0.933): Indica una correlación positiva muy fuerte y directa. En términos sencillos: a medida que aumenta el consumo de alcohol per cápita en un país, también aumenta la incertidumbre estadística de la medición.

  Distribución por Macro-Regiones:

  Europa: Se agrupa claramente en el extremo superior derecho. Es la región con mayor consumo, pero también donde los intervalos de confianza son más amplios (mayor dispersión).

  África y Asia: Se concentran en el extremo inferior izquierdo. Representan un grupo homogéneo de bajo consumo con mayor precisión estadística (intervalos más estrechos).

   América Latina: Se ubica en un rango medio, manteniendo la tendencia de la línea de regresión.

 El gráfico muestra que la variabilidad no es constante. A mayores niveles de consumo, los puntos se alejan más de la línea de regresión, lo que sugiere que estimar el consumo en países con alta ingesta es técnicamente más complejo o propenso a errores de reporte.

    Consistencia Técnica: El R=0.933 valida que el modelo de reporte es consistente: la incertidumbre es una función directa del volumen. Si hubiera puntos muy alejados de esta tendencia (outliers), indicarían errores de medición específicos en esos países./
    
\section{Diagnóstico de la Influencia de Valores Atípicos}

Para evidenciar estadísticamente la presencia de valores atípicos que distorsionan el promedio regional, se evaluó un modelo de regresión de la incertidumbre. El gráfico de Residuos vs Ajustados permite identificar a aquellos países (outliers) que se alejan de la varianza normal, justificando la necesidad de aplicar una técnica de truncamiento muestral\
$$ \text{ci\_width} = \beta_0 + \beta_1(\text{Consumo\_Per\_Capita}) + \epsilon $$
```{r grafico-residuos-region, fig.height=6, fig.width=9}
#datos
modelo_consistencia <- lm(ci_width ~ alcohol_liters_per_capita, data = clean_data_alcohol)

#extremos y para evitar errores
ajustados <- fitted(modelo_consistencia)
residuos <- residuals(modelo_consistencia)

#para los colores lidix
paleta_ecca <- c("#800000", "#1A5276", "#5B2C6F", "#7D3C98", "#2E86C1", "#1B4F72")
regiones <- as.factor(clean_data_alcohol$Region)
colores_puntos <- paleta_ecca[as.numeric(regiones)]
...
plot(ajustados, residuos,
     main = "Análisis de Residuos por Región",
     xlab = "Valores Ajustados (Predicción)",
     ylab = "Residuos (Error)",
     col = colores_puntos, 
     pch = 16, 
     cex = 1.2,
     ...
     col.lab = "#800000",   
     fg = "#800000",
     frame.plot = FALSE) #sin el marco

#agg la linea de referencia
abline(h = 0, lty = 2, col = "gray")

#borde
box(col = "#800000", lwd = 2)

legend("topright", 
       inset = c(-0.42, 0), 
       legend = levels(regiones), 
       fill = paleta_ecca, 
       title = "Macro-Regiones",
       bty = "n", 
       cex = 0.8)
```
```{r grafico-qqplot-region, fig.height=6, fig.width=9}
#extraemos los residuos estandarizados
residuos_std <- rstandard(modelo_consistencia)
# Calculamos los cuantiles teóricos de la distribución normal
orden <- order(residuos_std)
n <- length(residuos_std)
cuantiles_teoricos <- qnorm(ppoints(n))[order(orden)]

# Usamos la misma paleta y regiones que el gráfico anterior
paleta_ecca <- c("#800000", "#1A5276", "#5B2C6F", "#7D3C98", "#2E86C1", "#1B4F72")
regiones <- as.factor(clean_data_alcohol$Region)
colores_puntos <- paleta_ecca[as.numeric(regiones)]

par(mar = c(5, 5, 4, 10), xpd = TRUE) #cofiguracion de area

#grafico de normalidad
plot(cuantiles_teoricos, residuos_std,
     main = "Normal Q-Q Plot por Región",
...
legend("topright", 
       ...
       cex = 0.8)
```
Análisis de Residuos por Región

Este gráfico mide la diferencia entre los valores observados y los que el modelo predice (errores).
Falta de Linealidad (Patrón en "U" invertida): Los residuos no están distribuidos de forma aleatoria alrededor de la línea cero. Se observa una curvatura clara: el modelo subestima los valores en el rango medio (residuos positivos) y sobreestima en los extremos (residuos negativos al inicio). Esto confirma que la relación entre el consumo de alcohol y la incertidumbre no es puramente lineal, sino posiblemente logarítmica o polinómica.

Confirmación de Heterocedasticidad: La dispersión de los puntos aumenta considerablemente a medida que los valores ajustados crecen (hacia la derecha). Esto significa que el modelo es muy preciso para países con bajo consumo, pero pierde mucha fiabilidad en países con alto consumo (principalmente en la región de Europa, representada en azul claro).
Agrupación Regional: Se nota que los errores no son uniformes entre regiones. África y Asia (tonos oscuros/púrpura) están más concentrados, mientras que Europa muestra una varianza mucho mayor, sugiriendo que el modelo actual no captura las particularidades de esta región.

2. Gráfico Q-Q Normal por Región
Este gráfico evalúa si los errores del modelo siguen una distribución normal, un supuesto básico para que las pruebas de significancia sean válidas.
Desviación en las Colas (Heavy Tails): Aunque en la parte central los puntos siguen la línea punteada, se observa una desviación importante en el extremo superior derecho.

Presencia de Outliers: Los puntos que se disparan hacia arriba al final de la distribución indican la presencia de "valores atípicos" o países donde la incertidumbre es mucho más alta de lo que incluso el alto consumo de alcohol sugeriría.

Interpretación de Normalidad: La distribución de los errores tiene un sesgo positivo. Esto implica que el modelo tiende a cometer errores sistemáticos grandes en ciertos casos, lo que puede invalidar los intervalos de confianza si no se corrige./

\section{Análisis de Valores Atípicos y su Impacto en los Promedios Regionales}
...
```{r grafico-boxplot-final}
regiones_ordenadas <- clean_data_alcohol %>%
  group_by(Region) %>%
  summarise(Media = mean(alcohol_liters_per_capita, na.rm = TRUE)) %>%
  arrange(Media) %>%
  pull(Region)

#construcción del gráfico Pro :)
ggplot(clean_data_alcohol, aes(x = factor(Region, levels = regiones_ordenadas), 
                               y = alcohol_liters_per_capita)) +
  
  #guía visual de fondo 
  geom_segment(aes(x = Region, xend = Region, y = 0, yend = max(alcohol_liters_per_capita, na.rm = TRUE)), 
               color = "gray95", size = 5) +
  
  #puntos con Jitter y degradado pros
  geom_jitter(aes(color = alcohol_liters_per_capita), 
              width = 0.15, alpha = 0.4, size = 2) +
  
  #boxplot estilizado :)
  geom_boxplot(fill = "white", color = "#1A5276", alpha = 0.7, 
               outlier.color = "#800000", outlier.shape = 16, 
               outlier.size = 2, width = 0.4) +
  
  #un diamante en el promedio
  stat_summary(fun = mean, geom = "point", shape = 18, size = 5, 
               color = "#800000") +
  
  #escala de colores
  scale_color_gradient(low = "#1A5276", high = "#800000") +
  
  #etiquetas 
  labs(
    title = "Asimetría y Valores Atípicos Regionales",
    subtitle = "Distribución del Consumo | Comparativa Mediana (línea) vs. Promedio (diamante)",
    x = NULL,
    y = "Consumo Per Cápita (Litros)",
    caption = "Nota: El desplazamiento del diamante respecto a la mediana indica el impacto de los valores extremos.",
    color = "Litros"
  ) +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 16, color = "#800000"),
    plot.subtitle = element_text(size = 11, color = "gray30", margin = margin(b = 20)),
    axis.text.x = element_text(face = "bold", size = 10, color = "black", angle = 30, hjust = 1),
...
```{r grafico-comparativo-medias-corregido, fig.height=6, fig.width=9}
#calculos
media_completa <- mean(clean_data_alcohol$alcohol_liters_per_capita, na.rm = TRUE)
media_truncada <- mean(muestra_truncada_p90$alcohol_liters_per_capita, na.rm = TRUE)

#datos 
datos_largos <- bind_rows(
  clean_data_alcohol %>% mutate(Grupo = "1. Muestra Completa (n=940)"),
  muestra_truncada_p90 %>% mutate(Grupo = "2. Muestra Truncada (n=846)")
)

#tablita usando 2 decimales
datos_medias <- data.frame(
  Grupo = c("1. Muestra Completa (n=940)", "2. Muestra Truncada (n=846)"),
  MediaVal = c(media_completa, media_truncada),
  Etiqueta = paste("Media:", round(c(media_completa, media_truncada), 2))
)

#grafica
ggplot(datos_largos, aes(x = Grupo, y = alcohol_liters_per_capita, fill = Grupo)) +
  geom_violin(trim = FALSE, alpha = 0.2, color = NA) +
  #botblox central
  geom_boxplot(width = 0.25, color = "#800000", alpha = 0.6, outlier.shape = NA) +
  #diamante en la media
  geom_point(data = datos_medias, aes(x = Grupo, y = MediaVal), 
             color = "#800000", shape = 18, size = 5) +
  #etiqueta para los numeros
  geom_text(data = datos_medias, aes(x = Grupo, y = MediaVal + 0.5, label = Etiqueta), 
            color = "#800000", fontface = "bold", size = 4.5) +
  scale_fill_manual(values = c("#FADBD8", "#A9CCE3")) + #colores
  
  labs(title = "Impacto del Truncamiento P90 en la Tendencia Central",
       subtitle = "Reducción del sesgo por valores atípicos superiores",
       x = NULL, y = "Consumo (Litros/Cápita)") +
  
  theme_minimal(base_size = 11) +
  theme(
    panel.background = element_rect(fill = "#FFF5F5", color = NA),
    plot.background = element_rect(fill = "#FFF5F5", color = NA),
    legend.position = "none",
    plot.title = element_text(face = "bold", color = "#800000", size = 14),
    axis.text.x = element_text(face = "bold", color = "#333333"),
    panel.border = element_rect(color = "#800000", fill = NA, size = 1.2),
...
Este gráfico permite visualizar el éxito de la limpieza de datos. En la muestra original, la "cola" de la distribución se extendía demasiado hacia arriba, lo que hacía que el promedio (la media) se situara en 5.50 litros, una cifra que no representaba a la mayoría de los países. Al aplicar el truncamiento del 10% superior, logramos una distribución más equilibrada donde la media bajó a 4.71 litros.
    ...
    Conclusión: La limpieza eliminó el "ruido" de los países con consumos extremos, permitiendo que el promedio ahora sí sea un reflejo fiel de la realidad global./
    
```{r grafico-boxplot-truncado}
#calculamos 
regiones_ordenadas_p90 <- muestra_truncada_p90 %>%
  group_by(Region) %>%
  summarise(Media = mean(alcohol_liters_per_capita, na.rm = TRUE)) %>%
  arrange(Media) %>%
  pull(Region)
...
#grafico
ggplot(muestra_truncada_p90, aes(x = factor(Region, levels = regiones_ordenadas_p90), 
                                 y = alcohol_liters_per_capita)) +
  geom_segment(aes(x = Region, xend = Region, y = 0, yend = max(alcohol_liters_per_capita, na.rm = TRUE)), 
               color = "gray95", size = 5) +
  ...
  #boxplot estilizado 
  geom_boxplot(fill = "white", color = "#1A5276", alpha = 0.7, 
               outlier.shape = NA, # Ya no debería haber outliers según el nuevo P90
               width = 0.4) +
  ...
  #etiquetas
  labs(
    title = "Distribución Regional del Consumo (Muestra Depurada P90)",
    subtitle = "Análisis de tendencia central sin sesgo de valores extremos",
    x = NULL,
    y = "Consumo Per Cápita (Litros)",
    caption = "Nota: El diamante (promedio) se observa más alineado a la mediana tras el truncamiento del 10% superior.",
    color = "Litros"
  ) +
...
```
Impacto del Truncamiento en la Tendencia Central
El primer gráfico (Boxplot comparativo) es una prueba clara de la sensibilidad de la media aritmética ante valores extremos.
Reducción del Sesgo: Al eliminar el 10% de los países con mayor consumo (pasando de n=940 a n=846), la media descendió de 5.50 a 4.71 litros per cápita. Esta diferencia de 0.79 litros parece pequeña, pero a escala poblacional representa una magnitud masiva de alcohol no consumido por el ciudadano "promedio".
...
2. Distribución Regional (Muestra Depurada P90)
...
Heterogeneidad Regional Extrema:
Europa y Otras/Oceanía: Muestran cajas situadas en la parte alta del eje Y (consumo masivo). En Europa, incluso tras quitar el P90, la mayoría de los países consumen entre 7 y 10 litros.
África y Asia: Presentan una base muy ancha en los niveles bajos (puntos azules cerca del cero), pero con "colas" largas hacia arriba. Aquí, la media (diamante) está notablemente por encima de la mediana, lo que indica que una minoría de países en estas regiones consume mucho más que el resto.
...
\textbf{1.-} Heterogeneidad Global: El consumo de alcohol no sigue una distribución normal (Gaussiana) a nivel global ni en la mayoría de las regiones. Tratar los datos asumiendo normalidad llevaría a errores inferenciales.

Europa lidera el consumo: Es la única región donde la distribución masiva de los datos se agrupa en el extremo superior de la escala de consumo.
...
\textbf{Recomendación} 
El problema de los promedios en LATAM y Oceanía: Se desaconseja utilizar la "Media" para describir a América Latina y el Caribe u Oceanía en reportes ejecutivos. Al ser bimodales, afirmar que el país promedio consume "X" litros es una falacia ecológica; es mejor reportar que existen dos perfiles de consumo distintos en dichas zonas..\

\textbf{2.-} El análisis de los datos del período 2015-2019 permite segmentar el mundo en tres clústeres estadísticamente diferenciados respecto a su consumo per cápita de alcohol puro: un clúster de alto consumo (Europa, América del Norte y Oceanía), un clúster de consumo moderado (América Latina y el Caribe) y un clúster de bajo consumo (África y Asia). Las variaciones intra-clúster (dentro de cada grupo) se consideran producto de la variabilidad muestral o aleatoria, mientras que las variaciones inter-clúster (entre los grupos) representan diferencias poblacionales reales y significativas\

\textbf{Recomendación} 
1. Estratificación de Políticas por Clúster Regional
...
Se recomienda que las organizaciones globales de salud (como la OMS/OPS) no apliquen estrategias genéricas de control de consumo.
    ...
    Para el Clúster de Alto Consumo (Europa, Norteamérica, Oceanía): Dado que la diferencia entre sus medias no es significativa, se pueden estandarizar campañas transnacionales centradas en la reducción de daños crónicos, ya que el comportamiento de consumo es homogéneo y estructuralmente elevado.

    Para el Clúster de Bajo Consumo (África, Asia): El enfoque debe ser preventivo para evitar el desplazamiento de la media hacia niveles superiores debido a la apertura de mercados y cambios demográficos.

2. Monitoreo Especial de la Incertidumbre en LATAM
...
Dada la posición aislada de América Latina y el Caribe en la escala global, se recomienda fortalecer los sistemas de vigilancia epidemiológica locales.
    ...
    Es vital reducir el ancho de los intervalos de confianza (ci_width) en esta región, ya que su bimodalidad (detectada en el análisis de densidad) sugiere que los promedios regionales podrían estar ocultando realidades críticas en países específicos que no se ajustan al "clúster moderado".

3. Mejora en la Precisión de Reportes (Incertidumbre Técnica)
...
Una recomendación metodológica clave es incentivar la transición de reportes basados en estimaciones a reportes basados en registros administrativos reales.
    ...
    Se observa que las regiones con mayor sesgo (como Asia) suelen coincidir con una mayor variabilidad en la precisión de los datos. Reducir la incertidumbre técnica permitirá que los clústeres identificados en este estudio sean aún más precisos en futuras investigaciones.

4. Uso de la Mediana como Indicador de Seguimiento

Para futuros análisis de estos mismos datos, se recomienda adoptar la Mediana como indicador oficial de comparación para los clústeres de África y Asia.
...
\textbf{3.-} Dominio Europeo: Europa no solo lidera el consumo, sino que es la región que define la pendiente de la incertidumbre global en este estudio.\

\textbf{Recomendación} Se recomienda investigar los puntos que se alejan significativamente de la línea de regresión (especialmente en el rango de 8 a 12 litros) para entender por qué algunos países tienen mucha más incertidumbre que otros con el mismo nivel de consumo.\
...
\textbf{4.-} Correlación entre Consumo e Incertidumbre: Se evidencia que la precisión de las estimaciones es inversamente proporcional al volumen de consumo. El análisis de los intervalos de confianza ($ci\_width$) mediante modelos de regresión lineal confirma la presencia de heterocedasticidad: a mayor consumo per cápita, mayor es la varianza residual. Esto implica que los datos en regiones de alta ingesta presentan una volatilidad estructural que los indicadores puntuales no logran capturar.
...
\textbf{Recomendación} 
Hacia una Estadística No Paramétrica: Dada la persistencia de distribuciones asimétricas y bimodales detectadas, se recomienda que futuros informes de la Escuela incorporen pruebas de Wilcoxon o Kruskal-Wallis en lugar de ANOVA tradicional. Esto permitirá validar las diferencias entre clústeres regionales sin violar los supuestos de normalidad y homocedasticidad que este informe ha demostrado como inexistentes en la muestra global.
...
\chapter{Ranking de Países por Incertidumbre de Reporte}
...
```{r}
top_incertidumbre <- clean_data_alcohol %>%
  select(country, Region, alcohol_liters_per_capita, ci_width) %>%
  arrange(desc(ci_width)) %>%
  head(20)

kable(top_incertidumbre, 
      format = "latex", 
      booktabs = TRUE, 
      col.names = c("País", "Macro-Región", "Consumo", "Ancho Intervalo"),
      caption = "Top 20 Países con Mayor Incertidumbre Estadística") %>%
  kable_styling(
...
\chapter{Visualización de Densidad de Datos por Región}
...
``` {r}
#se agrupa por región y promedio de consumo 
heatmap_alcohol <- clean_data_alcohol %>%
  mutate(Consumo_Bin = cut(alcohol_liters_per_capita, breaks = 5)) %>%
  group_by(Region, Consumo_Bin) %>%
  summarise(N_Paises = n())

ggplot(heatmap_alcohol, aes(x = Region, y = Consumo_Bin, fill = N_Paises)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "#f9ebea", high = "#922b21", name = "N° de Países") +
  labs(x = "Macro-Región", y = "Rango de Consumo (Litros)", 
       subtitle = "Densidad de países: Nótese el solapamiento en niveles bajos para África/Asia") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```

\chapter{Identificación de Países con Consumo Extremo}
...
```{r anexo-lista-excluidos}
#identificamos los paises con valores atipicos
lista_excluidos <- clean_data_alcohol %>%
  anti_join(muestra_truncada_p90, by = "country") %>% # Ajusta si tu columna se llama 'Pais'
  select(Region, country, alcohol_liters_per_capita) %>%
  arrange(desc(alcohol_liters_per_capita)) #ordenados del más bebedor al menos

#renombramos columnas
colnames(lista_excluidos) <- c("Región", "País", "Consumo (L/Cápita)")

#tabla 
aplicar_estilo_tabla(lista_excluidos, 
                     "Países Excluidos del Análisis por Consumo Extremo (>11.25 L)") %>%
  kableExtra::kable_styling(bootstrap_options = c("striped", "hover"), 
                            full_width = F) %>%
  kableExtra::scroll_box(width = "100%", height = "400px") #si la lista es muy larga (94 países), usamos scroll para que no ocupe 5 páginas
```

\chapter*{Referencias Bibliográficas}

En esta sección se detallan las bases teóricas, metodológicas y computacionales que sustentan el presente análisis. Las fuentes seleccionadas abarcan desde los fundamentos del análisis exploratorio hasta las investigaciones globales sobre el tratamiento de la incertidumbre en salud pública y mercados.

## Bases de Estadística y Probabilidad

* **Tukey, J. W. (1977).** *Exploratory Data Analysis*. Addison-Wesley. 
    > (Referencia fundamental para el desarrollo del EDA y la visualización de datos mediante diagramas de caja y bigotes). [@tukey1977exploratory]
    
* **Huber, P. J. (1981).** *Robust Statistics*. John Wiley & Sons. 
    > (Sustento teórico para el manejo de valores atípicos y la crítica a la sensibilidad de la media aritmética). [@huber1981robust]
    
* **Spiegelhalter, D. (2019).** *The Art of Statistics: Learning from Data*. Pelican Books. 
    > (Obra clave para entender la comunicación del riesgo y la tiranía de los promedios en la toma de decisiones). [@spiegelhalter2019art]

* **Huff, D. (1954).** *How to Lie with Statistics*. W. W. Norton & Company. 
    > (Validación sobre la manipulación política y social de los indicadores estadísticos centrales). [@huff1954how]

* **Pareto, V. (1896).** *Cours d'économie politique*. Rouge. 
    > (Base teórica para el análisis de la distribución de ventas y la regla del 80/20 observada en el mercado de videojuegos). [@pareto1896cours]

## Investigaciones y Casos de Estudio

* **World Health Organization (2018).** *Global status report on alcohol and health 2018*. 
    > (Referencia para el estudio de indicadores transversales y la complejidad de las estimaciones per cápita). [@who2018alcohol]

* **GBD 2016 Alcohol Collaborators (2018).** *Alcohol use and burden for 195 countries and territories*. The Lancet. 
    > (Investigación que ilustra el fenómeno de solapamiento de intervalos de confianza en comparaciones internacionales). [@gbd2018alcohol]

* **Van der Bles, A. M., et al. (2019).** *Communicating uncertainty about facts, numbers and science*. Royal Society Open Science. 
    > (Estudio sobre la importancia técnica de reportar la variabilidad y el error en los datos públicos). [@vanderbles2019communicating]

## Fundamentos de Computación y Análisis de Datos

* **Wickham, H., et al. (2019).** *Welcome to the Tidyverse*. Journal of Open Source Software. 
    > (Referencia técnica para el procesamiento de datos bajo el paradigma de programación funcional en R). [@wickham2019welcome]

* **Wilke, C. O. (2019).** *Fundamentals of Data Visualization*. O'Reilly Media. 
    > (Guía para el diseño de visualizaciones que representen fielmente la densidad de probabilidad y la incertidumbre). [@wilke2019fundamentals]

* **Little, R. J. A., & Rubin, D. B. (2019).** *Statistical analysis with missing data*. John Wiley & Sons. 
    > (Tratamiento avanzado de valores faltantes y el impacto del sesgo de selección en bases de datos masivas). [@little2019statistical]