...
\item En el \textbf{Capítulo I}, se plantea la problemática del impacto de los valores extremos en la salud pública, formulando las preguntas de investigación sobre la representatividad del promedio y delimitando el alcance del estudio al periodo 2015-2019.
    ...
    \item El \textbf{Capítulo III} describe el Marco Metodológico y Computacional, detallando el protocolo de limpieza y carga en \textbf{R}, así como la implementación del algoritmo de truncamiento por el percentil 90 (P90) para el control de la varianza.
...
tabla_limpiieza <- bind_rows(tendencia_pre, tendencia_post) %>%
  mutate(Sesgo_Absoluto = Media - Mediana)

aplicar_estilo_tabla(tabla_limpiieza, "Comparación post-limpieza de los datos")
``` 

No se encontraron valores faltantes en la muestra seleccionada 

\subsubsection{Simulación de Fallos y Validación del Protocolo de Limpieza}
...
\subsubsection{Protocolo de Recuperación de Datos}
...
\textbf{1.-} El consumo de alcohol no sigue una distribución normal (Gaussiana) a nivel global ni en la mayoría de las regiones. Tratar los datos asumiendo normalidad llevaría a errores inferenciales.
...
\textbf{3.-} Europa no solo lidera el consumo, sino que es la región que define la pendiente de la incertidumbre global en este estudio.

\textbf{Recomendación} Se recomienda investigar los puntos que se alejan significativamente de la media (especialmente en el rango de 8 a 12 litros) para entender por qué algunos países tienen mucha más incertidumbre que otros con el mismo nivel de consumo.
...
\chapter{Anexo Diagnóstico de la Influencia de Valores Atípicos}
